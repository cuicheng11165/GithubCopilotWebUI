...
| Docker Desktop | 团队部署、需要容器隔离 | SQLite，或 PostgreSQL + Redis | Linux 容器隔离 | 可通过端口或反向代理访问 |

如果只是先运行起来，建议从“Windows 原生部署”开始。如果会有不受完全信任的用户，或需要从其他机器访问，请使用 Docker Desktop，并优先选择多用户模式。
...
SQLite 数据保存在项目的 `data` 目录，Redis 数据保存在 Docker named volume。默认 Worker 并发为 2，SQLite 仍然只有一个写入者，不建议在未压测时提高并发。
...
不要使用 `docker compose down -v`，除非明确希望删除 Redis 等 named volume 中的数据。

### 3.5 PostgreSQL 多用户模式

需要更高并发或多副本运行时，在 `.env` 中设置强随机密码：

```dotenv
POSTGRES_PASSWORD=<强随机密码>
MULTI_USER_WORKER_CONCURRENCY=8
```

启动：

```powershell
docker compose -f compose.multi-user.yaml config
docker compose -f compose.multi-user.yaml up --build -d
docker compose -f compose.multi-user.yaml ps
docker compose -f compose.multi-user.yaml logs -f --tail 200
```

该模式使用 PostgreSQL、Redis/BullMQ 和 `copilot-state` named volume。SQLite 和 PostgreSQL 模式之间不会自动复制数据。
...
恢复时，将这些文件放回相同位置，重新运行 `corepack pnpm db:migrate:local`，然后启动服务。

### 4.2 多用户模式备份

至少备份：

- PostgreSQL 数据库（使用 `pg_dump` 或企业数据库备份工具）。
- `copilot-state` named volume。
- `.env` 和仓库配置。

Redis 主要承担队列、锁和事件协调，仍建议按组织的恢复目标配置持久化和备份。
...
- [ ] 已测试命令审批、停止和超时处理。
- [ ] 已制定 `data` 或 PostgreSQL/Copilot state 的备份策略。
- [ ] 对外部署已启用 HTTPS，并限制 API、数据库和 Redis 的网络访问。
